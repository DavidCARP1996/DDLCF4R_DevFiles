...
init python:
    ...
    def show_doki(t,action = "hop"):
        if t.sPoint >= 3 and bag_inventory.has_item("sbook1"):
            renpy.show("s_sticker " + action)
        if t.nPoint >= 3 and bag_inventory.has_item("nbook1"):
            renpy.show("n_sticker " + action)
        if t.yPoint >= 3 and bag_inventory.has_item("ybook1"):
            renpy.show("y_sticker " + action)
        if t.mPoint >= 3 and bag_inventory.has_item("mbook1"):
...
label poem(transition=True):
    ...
    show screen poemgame_hud #Muestra el nuevo Hud del minijuego del poema :3
    #Create the character stickers
    if persistent.playthrough == 3:
        show m_sticker at sticker_m
    else:
        if in_sayori_kill == None: #Show Sayori in Act 1
